...
class EnhancedSearchOrchestrator:
    ...
    async def search(
        ...
        answer_style: str = "full"  # "full" (по умолчанию) или "brief"
    ) -> SearchResult:
        """
        ...
        steps = []
        # Стиль ответа: 'brief' отключает требования развёрнутости.
        # Нужен, чтобы отделить длину ответа от качества памяти —
        # на бенчмарке короткой фактической точности развёрнутый ответ
        # проигрывает судье системно.
...
"""
        ...
        # В brief-режиме доказательный контракт не подключается: он требует
        # четырёх секций (вывод → прямые основания → интерпретация →
        # неопределённости), то есть ровно той структуры, которую brief
        # снимает. Без этого режим противоречил сам себе и краткости не давал.
        if getattr(self, "_answer_style", "full") == "brief":
            pass
        elif (
            search_depth != "quick"
...
"""
                ...
                # «Не придумывай evidence» осталось, но больше не читается как
                # разрешение промолчать: раньше эта строка соседствовала с
                # «Лучше частичный ответ, чем отказ отвечать», и на практике
                # побеждала она. В замере это давало 26 отказов из 60, причём
                # на 17 из них ответ доставал обычный BM25 — то есть данные в
                # памяти были. Теперь отказ явно привязан к ОТСУТСТВИЮ
                # относящегося к вопросу контекста, а не к неполноте оснований.
                evidence_contract_instructions = """
...
- Если есть пробелы, противоречия или слабые места в данных — выдели их отдельным блоком "Неопределённости".
- Не придумывай evidence. Но неполнота оснований — НЕ повод для отказа: дай ответ по тому, что есть, и вынеси недостающее в "Неопределённости". Отказ уместен, только если в контексте нет НИЧЕГО по теме вопроса.
"""
        ...
        # Язык ответа. Инструкция #7 была прибита к русскому при заявленной
        # двуязычности: на английский вопрос приходил русский ответ. Наивная
        # проверка «есть ли хоть один кириллический символ» тоже не годится —
        # она заворачивает в русский любой английский вопрос с русским именем
        # собственным («What did Титан-Банк decide?»), а для двуязычной
        # компании это обычный запрос.
        #
        # Решает преобладающий алфавит (общий помощник
        # backend/core/llm/lang.py), а при неясной смеси — язык интерфейса
        # пользователя, то есть штатная политика продукта. Порядок именно
        # такой: человек, написавший по-английски, ждёт английского ответа
        # независимо от сохранённой локали.
        from backend.core.llm.lang import (query_script_lang,
                                           resolve_answer_lang)
        # Стиль ответа. По умолчанию продукт пишет развёрнуто — это осознанный
        # выбор для человека, читающего аналитику. Режим "brief" существует,
        # чтобы отделить ДЛИНУ ответа от КАЧЕСТВА памяти: судья LongMemEval для
        # части типов вопросов инструктирован отвечать «нет», если ответ
        # содержит лишь часть требуемого, поэтому развёрнутый ответ проигрывает
        # системно, независимо от того, что нашла память.
        _brief = getattr(self, "_answer_style", "full") == "brief"
        _style_len_instruction = (
            "Отвечай КРАТКО и по существу: только прямой ответ на вопрос, "
            "без вступлений и пересказа контекста."
            if _brief else
            "Ответ должен быть ПОДРОБНЫМ и РАЗВЁРНУТЫМ - извлеки максимум полезной информации.")
        _style_fmt_instruction = (
            "Не используй заголовки и списки — ответ в одну-две фразы."
            if _brief else
            "Структурируй ответ с помощью списков и заголовков, если информации много.")

        _lang = query_script_lang(query)
        if not _lang:
            try:
                _lang = await resolve_answer_lang(user_id or "")
            except Exception:
                _lang = "ru"
        _answer_lang_instruction = (
            "Отвечай на русском языке." if _lang == "ru"
            else f"Answer in the SAME language as the question ({_lang}).")
...
Формулировки вида "недостаточно оснований", "cannot determine", "информация недоступна" допустимы ТОЛЬКО когда в контексте нет ни одного факта по теме вопроса. Если относящиеся к теме факты есть, но их не хватает для полного ответа - отвечай тем, что есть, и честно назови, чего именно недостаёт.
"""
...
"""
        ...
        # Хвост user_prompt тоже обязан слушаться режима: он был безусловным
        # («дай подробный ответ»), то есть в brief промпт требовал краткости
        # системной частью и развёрнутости — пользовательской. Замер: brief
        # давал промпт даже на символ ДЛИННЕЕ full, то есть режим не работал.
        _style_ask = (
            "Ответь на вопрос по контексту выше. Коротко: одна-две фразы, "
            "только суть, без вступлений."
            if _brief else
            "Пожалуйста, дай подробный ответ на вопрос, используя информацию "
            "из контекста выше.")
