...
def query_script_lang(text: str | None) -> str:
    """Язык ТЕКСТА по преобладающему алфавиту: "ru", "en" или "" (неясно).

    Нужен там, где язык надо взять из самого вопроса, а не из локали: у
    фоновой задачи и у бенчмарка ни запроса, ни персоны нет, и
    `resolve_answer_lang` отдаст дефолт стенда — русский — даже на
    английский вопрос.

    Считается ДОЛЯ, а не факт наличия. Прежняя проверка в оркестраторе
    поиска («есть хоть один кириллический символ») заворачивала в русский
    ответ любой английский вопрос с русским именем собственным — а для
    двуязычной компании «What did Титан-Банк decide?» это обычный запрос.

    Порог 0.6 намеренно не 0.5: при примерно равной смеси язык не выбирается
    (возвращается ""), и вызывающий уходит к локали пользователя, вместо
    того чтобы угадывать по одному символу перевеса.
    """
    cyr = lat = 0
    for ch in (text or ""):
        low = ch.lower()
